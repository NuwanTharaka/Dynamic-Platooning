...
RSU_JoinAtBack::RSU_JoinAtBack(GeneralPlatooningApp* app)
    : RSU_JoinManeuver(app)
    , RSU_joinManeuverState(RSU_JoinManeuverState::IDLE)
{
}

void RSU_JoinAtBack::startManeuver(const void* parameters)
{
    JoinManeuverParameters* pars = (JoinManeuverParameters*) parameters;
    if (RSU_joinManeuverState == RSU_JoinManeuverState::IDLE) {
        ...
        RSU_joinManeuverState = RSU_JoinManeuverState::J_WAIT_REPLY;
    }
}

void RSU_JoinAtBack::abortManeuver()
{
}

void RSU_JoinAtBack::onPlatoonBeacon(const PlatooningBeacon* pb)
{
    if (RSU_joinManeuverState == RSU_JoinManeuverState::J_MOVE_IN_POSITION) {
        // check correct role
        ...
        if (pb->getVehicleId() == frontId) {
            ...
            if (distance < 16) { // TODO fixed value? make dependent on
                ...
                RSU_joinManeuverState = RSU_JoinManeuverState::J_WAIT_JOIN;
            }
        }
    }
}

void RSU_JoinAtBack::handleJoinPlatoonRequest(const JoinPlatoonRequest* msg)  //set inmanuver == true //should add the functionality to queue multiple requests
{
    //go inside this only if rsu is idling
    Coord joinerPosition(msg->getXPos(),msg->getYPos()); //are they in omnett++ coordinate system or normal coordinate system
    Coord rsuPosition(2000,2000);                            //should Genaralize this 
    double distance = rsuPosition.distance(joinerPosition);  //is it in meters?

    if (distance < 200){

        //dont send the joinPlatoonResponse just yet
        joinerData.reset(new JoinerData());
        joinerData->from(msg);

        //send requests to platoon leaders to send their info
        RequestPlatoonInfo* request = createRequestPlatoonInfo(99999,"99999",99999,-1,"true");
        app->sendUnicast(request, -1);


        sendJoinPlatoonResponse_1 = new cMessage();
        scheduleAt(SimTime(10 +simTime()), sendJoinPlatoonResponse_1); //Wait 10 seconds to process platoonInfo messages 
...
void RSU_JoinAtBack::handlePlatoonInfo(const PlatoonInfo* msg)
{ /*
    ...
    //Write something to store the incmoing platoonInfo messages. Accept platoonInfo messages only from leaders in vicinity //store their platoon ID's 

}

void RSU_JoinAtBack::sendJoinPlatoonResponse()
{ /*
    if (app->getPlatoonRole() != PlatoonRole::JOINER) return;
    if (joinManeuverState != JoinManeuverState::J_WAIT_REPLY) return;
    if (msg->getPlatoonId() != targetPlatoonData->platoonId) return;
    ...
    //Process the platoonInfo messages
    bool permission = true; // if no platoonInfo msgs then this should be false

    JoinPlatoonResponse* response = createJoinPlatoonResponse(99999, "99999", 0, joinerData->joinerId, permission);
    app->sendUnicast(response, joinerData->joinerId);

    //tell the leader to send moveToPositionMessage.
    //JoinPlatoonResponse* response = createJoinPlatoonResponse(positionHelper->getId(), positionHelper->getExternalId(), msg->getPlatoonId(), msg->getVehicleId(), permission);
    //SendMTP* request = createSendMTP( joinerData->joinerId," joinerData->joinerId","Platoon ID","Leader ID",true); //should send joiner data too //putting the Joiner ID in vehicle 
    SendMTP* request = createSendMTP( joinerData->joinerId,0,1,1,true); 
...
void RSU_JoinAtBack::handleJoinPlatoonResponse(const JoinPlatoonResponse* msg)
{
    if (app->getPlatoonRole() != PlatoonRole::JOINER) return;
    if (RSU_joinManeuverState != RSU_JoinManeuverState::J_WAIT_REPLY) return;
    if (msg->getPlatoonId() != targetPlatoonData->platoonId) return;
    if (msg->getVehicleId() != targetPlatoonData->platoonLeader) return;
    ...
    if (msg->getPermitted()) {
        // wait for information about the join maneuver
        RSU_joinManeuverState = RSU_JoinManeuverState::J_WAIT_INFORMATION;
        // disable lane changing during maneuver
        traciVehicle->setFixedLane(traciVehicle->getLaneIndex());
    }
    else {
...
void RSU_JoinAtBack::handleMoveToPosition(const MoveToPosition* msg)
{
    if (app->getPlatoonRole() != PlatoonRole::JOINER) return;
    if (RSU_joinManeuverState != RSU_JoinManeuverState::J_WAIT_INFORMATION) return;
    if (msg->getPlatoonId() != targetPlatoonData->platoonId) return;
    if (msg->getVehicleId() != targetPlatoonData->platoonLeader) return;
    ...
    RSU_joinManeuverState = RSU_JoinManeuverState::J_MOVE_IN_POSITION;
}

void RSU_JoinAtBack::handleMoveToPositionAck(const MoveToPositionAck* msg)
{
    if (app->getPlatoonRole() != PlatoonRole::LEADER) return;
    if (RSU_joinManeuverState != RSU_JoinManeuverState::L_WAIT_JOINER_IN_POSITION) return;
    if (msg->getPlatoonId() != positionHelper->getPlatoonId()) return;
    ...
    RSU_joinManeuverState = RSU_JoinManeuverState::L_WAIT_JOINER_TO_JOIN;
}

void RSU_JoinAtBack::handleJoinFormation(const JoinFormation* msg)
{
    if (app->getPlatoonRole() != PlatoonRole::JOINER) return;
    if (RSU_joinManeuverState != RSU_JoinManeuverState::J_WAIT_JOIN) return;
    if (msg->getPlatoonId() != targetPlatoonData->platoonId) return;
    if (msg->getVehicleId() != targetPlatoonData->platoonLeader) return;
...
void RSU_JoinAtBack::handleJoinFormationAck(const JoinFormationAck* msg)
{
    if (app->getPlatoonRole() != PlatoonRole::LEADER) return;
    if (RSU_joinManeuverState != RSU_JoinManeuverState::L_WAIT_JOINER_TO_JOIN) return;
    if (msg->getPlatoonId() != positionHelper->getPlatoonId()) return;
    if (msg->getVehicleId() != joinerData->joinerId) return;
    for (unsigned i = 0; i < msg->getNewPlatoonFormationArraySize(); i++) ASSERT(msg->getNewPlatoonFormation(i) == joinerData->newFormation.at(i));
...
void RSU_JoinAtBack::handleSelfMsg(cMessage* msg)
{

    // this takes car of feeding data into CACC and reschedule the self message
    //BaseScenario::handleSelfMsg(msg);

    if (msg == sendJoinPlatoonResponse_1) sendJoinPlatoonResponse();
}